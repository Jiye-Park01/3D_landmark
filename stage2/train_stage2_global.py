# Stage 2 Training with Improved Global Feature Extraction
# Global Feature Extraction: Avg+Max+Std+GeM → MLP 512
...
def train_stage2(args):
    """Stage 2 학습 with Improved Global Feature Extraction
    
    New Global Feature Extraction Method:
    - Avg Pooling: 각 랜드마크의 평균 활성도
    - Max Pooling: 각 랜드마크의 최대 활성도  
    - Std Pooling: 각 랜드마크의 활성도 분산
    - GeM Pooling: 학습 가능한 일반화된 평균 (p=3.0 초기값)
    
    Input: Heatmaps (B, 68, N) → Pooled Features (B, 272) → MLP → (B, 512)
    
    Benefits:
    1. 정보 보존: 단순 평균 대신 다양한 통계량 활용
    2. 학습 가능: GeM의 p 파라미터가 학습됨
    3. 풍부한 특징: 272차원 → 512차원으로 의미있는 변환
    4. 다양한 표정 대응: 여러 통계량으로 복잡한 패턴 학습
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    
    print("="*60)
    print("Stage 2 Training with IMPROVED Global Feature Extraction")
    print("Method: Avg + Max + Std + GeM → MLP 512")
    print("Input: Heatmaps (B, 68, N)")
    print("Pooling: 4 × 68 = 272 features")
    print("Output: 512-dimensional global features")
    print("="*60)
    ...
    log_filepath = f'./checkpoints_global_feature/training_log_{timestamp}.txt'
    training_log_filepath = f'./checkpoints_global_feature/training_console_log_{timestamp}.txt'
    
    # 체크포인트 저장 경로
    checkpoint_dir = './checkpoints_global_feature'
    ...
    with open(training_log_filepath, 'w') as f:
        ...
        f.write("="*50 + "\n")
        f.write(f"IMPROVED Global Feature Extraction:\n")
        f.write(f"  - Method: Avg + Max + Std + GeM → MLP 512\n")
        f.write(f"  - Input: Heatmaps (B, 68, N)\n")
        f.write(f"  - Pooling: 4 × 68 = 272 features\n")
        f.write(f"  - MLP: 272 → 512 → 512 → 512\n")
        f.write(f"  - GeM Parameter: Learnable p (initial: 3.0)\n")
        f.write(f"  - Benefits: Better information preservation, learnable pooling\n")
        f.write(f"\nLoss configuration:\n")
    ...
    print(f"IMPROVED Global Feature Extraction:")
    print(f"  - Method: Avg + Max + Std + GeM → MLP 512")
    print(f"  - Input: Heatmaps (B, 68, N)")
    print(f"  - Pooling: 4 × 68 = 272 features")
    print(f"  - MLP: 272 → 512 → 512 → 512")
    print(f"  - GeM Parameter: Learnable p (initial: 3.0)")
    print(f"  - Benefits: Better information preservation, learnable pooling")
    print(f"\nLoss configuration:")
