...
def main():
    ...
    print("\nEvaluating all test samples to find best performing ones...")
    ...
    # 오차를 기준으로 정렬
    all_sample_results.sort(key=lambda x: x['error'])

    # 가장 오차가 낮은 상위 5개 샘플 선택
    # best_5_samples = all_sample_results[:min(5, len(all_sample_results))]
    best_5_samples = all_sample_results[:min(5, len(all_sample_results))]

    print(f"\nProcessing and saving results for the top {len(best_5_samples)} best performing samples:")
    # 두 번째 패스: 선택된 샘플에 대한 상세 결과 저장 및 시각화
    for i, sample_data in enumerate(best_5_samples):
        ...
        # 히트맵 시각화 및 저장
        heatmap_path = f'./results/heatmaps/best_sample_{i+1}_heatmap.png'
        ...
        # npy 파일로 저장
        save_path = f'./results/best_sample_{i+1}_results.npy'
